          ...
          <nav className="hidden md:flex items-center space-x-10">
            {navigation.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className={`text-base font-medium transition-all duration-300 hover:text-[#ff914d] px-2 py-1 rounded-lg ${
                  isActive(item.href)
              ...
              <SheetContent side="right" className="w-[85vw] max-w-sm bg-white/95 backdrop-blur-md border-l border-[#ff914d]/10">
                <div className="flex flex-col h-full px-6 py-8">
                  ...
                  <div className="flex items-center justify-center mb-12">
                    ...
                    />
                    <span className="text-2xl font-bold text-[#191919]">Meetix</span>
                  </div>
                    ...
                    {navigation.map((item) => (
                      <SheetClose asChild key={item.name}>
                        <Link
                          ...
                          className={`text-lg font-medium transition-all duration-300 py-3 px-4 rounded-xl ${
                            isActive(item.href)
                              ? "text-[#ff914d] bg-[#ff914d]/10"
